document.addEventListener('DOMContentLoaded', function () {
	...
	// категории
	let sectionCategories = searchResultsWrapper.querySelector(
			'.widget-search-results_categories'
		),
		categoriesList = sectionCategories.querySelector(
			'.widget-search-results_categories-list'
		)

	// товары
	let sectionProducts = searchResultsWrapper.querySelector(
			'.widget-search-results_products'
		),
		productsList = sectionProducts.querySelector(
			'.widget-search-results_products-list'
		)

	// история
	let sectionHistory = searchResultsWrapper.querySelector(
			'.widget-search-results_history'
		),
		historyList = sectionHistory.querySelector(
			'.widget-search-results_history-list'
		)

	// все результаты
	let sectionAllResults = searchResultsWrapper.querySelector(
			'.widget-search-results_all'
		),
		countCategoriesAllResults = sectionAllResults.querySelector(
			'.all-results_categories'
		),
		countProductsAllResults = sectionAllResults.querySelector(
	...
	function statusSearchResultWrapper() {
		...
		window.addEventListener('click', (element) => {
			if (
				searchResultsWrapper.classList.contains('active') &&
				element.target.className != 'widget-search' &&
				!element.target.closest('.widget-search')
			) {
		...
		function renderCategories(categoriesResult) {
			...
			categoriesResult.forEach((category) => {
				if (category.products_count > 0) {
					categories += `<div data-href='/catalog/${
						category.id
					}'><span data-level="${category.level}">${
						category.name
					}</span> <span data-level="${category.parent_level}">${
						category.parent_name
					}</span> <span>${category.products_count} ${endingWord(
						words,
	...
	btnsSearch.forEach((btn) =>
		btn.addEventListener(
			'click',
			() => (window.location = '/search?q=' + queryValid())
		)
	)
	...
	function clearHistorySearch() {
		let btnClear = sectionHistory.querySelector(
			'.widget-search-results_history-btn-clear'
		)
